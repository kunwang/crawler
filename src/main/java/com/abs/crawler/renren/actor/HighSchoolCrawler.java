...
/**
 * @author hao.wang
 * @since 2016/5/13 04:16
 */
@Actor
public class HighSchoolCrawler extends AbstractCrawler {

    private static final Logger LOGGER = LoggerFactory.getLogger(HighSchoolCrawler.class);

    private static final String TEMPLATE = "http://support.renren.com/highschool/%s.html?requestToken=486986603&_rtk=e8c7d1c5";
    ...
    public void onSuccess(CrawlerMessage message, Response response) throws IOException {
                ...
                if (StringUtils.equals(areaDetail.attr("style"), "display:none;")) {
                    ...
                    if (!schoolElements.isEmpty()) {
                        for (Element schoolElement : schoolElements) {
                            ...
                            highSchool.setId(id);
                            highSchool.setName(school);
                            highSchool.setCity(city);
                            highSchool.setProvince(province);
                            highSchool.setArea(areaName);

                            highSchoolRepository.saveOrUpdate(highSchool);
                            count ++;
        ...
        LOGGER.info("high school crawler,province={},city={},cityId={},count={}", province, city, cityId, count);
    }
